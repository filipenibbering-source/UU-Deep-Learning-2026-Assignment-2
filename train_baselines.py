...
from src.train import train_vae, train_baseline
...
from src.models.baselines import BaselineLSTM, BaselineResNet18, BaselineResNet50, BaselineEEGNet
...
def main(config_file: Optional[str] = None):
    ...
    assert config.is_baseline, "Model is not a baseline model"
    # config.learning_rate = 1e-4
    
    
    match config.model:
        case 'lstm':
            model = BaselineLSTM(input_size=248 * 64, hidden_size=config.hidden_size, num_layers=config.num_layers, dropout=config.dropout, bidirectional=config.bidirectional)
        case 'resnet18':
    ...
    train_baseline(model, config, run_name=config_file.split('/')[-1].split('.')[0] if config_file is not None else '')
