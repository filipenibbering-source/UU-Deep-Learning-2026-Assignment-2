...
from src.data import DataSet, DataSetType, SampleDataSet
...
def train_baseline(model: nn.Module, config: Config, run_name: str = ''):
    if run_name != '':
    ...
    if config.data_type.lower().strip() == 'intra':
        train_dataset = SampleDataSet(dataset_type=DataSetType.INTRA, split='train', data_transform='raw', prefix='')

        
        val_dataset = SampleDataSet(dataset_type=DataSetType.INTRA, split='test', data_transform='raw', prefix='')
        val_dataset.get_mean_and_std(train_dataset)
    else:
        train_dataset = SampleDataSet(dataset_type=DataSetType.CROSS, split='train', data_transform='raw', prefix='')
        val_dataset = SampleDataSet(dataset_type=DataSetType.CROSS, split='test1', data_transform='raw', prefix='')
        val_dataset.get_mean_and_std(train_dataset)
    ...
    model = BaselineModel(model, config)
